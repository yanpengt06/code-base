...
from dataloader import get_IEMOCAP_loaders, get_SST2_loaders
from transformers import AdamW, get_linear_schedule_with_warmup, AutoModelForSequenceClassification
import copy
from tutils import remove_layer_idx, get_param_group
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=512, metavar='BS', help='batch size')
    ...
    logger = get_logger(
        path + args.dataset_name + f'/{now}_b{batch_size}_e{n_epochs}_lr1_{lr_rbt}_lr2_{lr_o}_seed{seed}.log')
    logger.info('start training on GPU {}!'.format(os.environ["CUDA_VISIBLE_DEVICES"]))
    logger.info(args)

    train_loader, valid_loader, test_loader = get_SST2_loaders(
        dataset_name=args.dataset_name, batch_size=batch_size, num_workers=4)
    n_classes = 2
    ...
    model = AutoModelForSequenceClassification.from_pretrained('bert-base-uncased', num_lables=n_classes)
    # for n, p in model.named_parameters():
    #     print(n)
    """separate learning rate, set param groups here(Optional)"""
    ...
    optimizer = AdamW(model.parameters(), lr=args.lr_o)
    total_steps = n_epochs * len(train_loader)
    scheduler = get_linear_schedule_with_warmup(optimizer=optimizer, num_warmup_steps=int(total_steps * 0.06),
    ...
    for e in range(n_epochs):
        ...
        # is not 'DailyDialog' dataset, i.e. use weighted-average-f1-score as metric
        else:
